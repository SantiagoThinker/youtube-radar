...
if [ -z "$HANDLES" ]; then
    note "No channels entered — keeping the default channels.yaml that ships with the repo."
else
...
# Generate me.md from scratch using Python — embeds the template inline so
# there's no separate me.template.md file to keep in sync.
NAME="$NAME" \
...
background = os.environ.get("BACKGROUND", "").rstrip() or "(empty — re-run setup.sh to fill in)"
lenses = os.environ.get("LENSES_MD", "").rstrip()
stoplist = os.environ.get("STOPLIST", "").rstrip() or "(none yet — add patterns over time as the Synthesizer surfaces things you already know)"

content = f"""# {name} — profile for Synthesizer

Generated by setup.sh. Edit freely — the Synthesizer reads this before every
output.
...
The Synthesizer produces one section per lens for every video. Each lens can
refuse cleanly with a one-sentence reason if the content doesn't apply.
...
The Synthesizer filters these patterns as already-known noise:

{stoplist}

What **does** count as a valuable insight:
- Concrete numbers (revenue, conversion, latency, cost) from practitioners
- Names of companies / people worth subscribing to or contacting, with a reason
